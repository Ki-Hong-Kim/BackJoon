...
# 문제 이해
# 벽 3개를 세울 모든 경우의 수의 안전지역 수를 확인하고
# 그 중 가장 많은 안전지역 수를 알아보자
...
num_virus = 9999 # 바이러스 퍼진 구역 수를 최소값으로 업데이트 하기위한 vector (n * m) 보다 크기만 하면됨
safe = -3  # 벽을 3개 꼭 세워야함.
...
def solve(start, wall):  # 벽을 세운다 (start = 0, wall = 0)
    ...
    global n, m, num_virus, visited # n, m은 행렬 모형
                                    # num_virus = 계속 업데이트 되는 바이러스 구역 최소 값
                                    # visited = 방문여부 확인 테이블
